...
export default function CustomerPublicPortal() {
  ...
  const [isProcessingOrder, setIsProcessingOrder] = useState(false); // Estado de Carga Nuevo
  ...
  const addToCart = (item: any) => {
    setCart(prev => {
      ...
      if (exists) return prev.map(cartItem => cartItem.item.id === item.id ? { ...cartItem, quantity: cartItem.quantity + 1 } : cartItem);
      return [...prev, { item, quantity: 1 }];
    });
  };

  const removeFromCart = (itemId: string) => setCart(prev => prev.filter(cartItem => cartItem.item.id !== itemId));
  ...
  // ==========================================
  // MOTOR DE CHECKOUT TIER 1 (NUEVA VERSIÓN)
  // ==========================================
  const handleCheckout = async () => {
    ...
    setIsProcessingOrder(true); // Activa el Loader
    
    try {
      let generatedPaymentLink = "";

      // 1. GENERACIÓN DEL LINK DINÁMICO EN MERCADO PAGO
      if (paymentMethod === 'link') {
        ...
        if (data.init_point) {
          generatedPaymentLink = data.init_point;
        } else {
          alert("Hubo un ligero retraso generando tu link seguro. Se procesará manual en caja.");
        }
      }

      // 2. INYECCIÓN AL KDS DE COCINA
      ...
      await setDoc(orderRef, {
        ...
        paymentLink: generatedPaymentLink || null, // Guardamos el link en BD por si el dueño quiere reenviarlo
        createdAt: new Date().toISOString()
      });
      
      // 3. CONSTRUCCIÓN LIMPIA DEL MENSAJE DE WHATSAPP (Con saltos de línea \n)
      let orderText = `*NUEVO PEDIDO - ${businessData.businessName}*\n\n`;
      
      cart.forEach(c => {
        orderText += `${c.quantity}x ${c.item.name} - $${c.item.price * c.quantity}\n`;
      });
      ...
      if (paymentMethod === 'link') {
        orderText += `*Método de Pago:* 📱 Link Mercado Pago\n`;
        if (generatedPaymentLink) {
          orderText += `👉 *PAGAR AQUÍ:* ${generatedPaymentLink}\n`;
        }
      }

      if (customerProfile) {
        orderText += `\n*Cliente VIP:* ${customerProfile.name} (Tel: ${customerProfile.phone})`;
      }

      // 4. CODIFICACIÓN CRÍTICA (Evita que el texto y emojis se rompan)
      ...
      setIsProcessingOrder(false); // Apaga el Loader
    }
  };
          ...
          <div className="bg-white w-full max-w-md rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden relative max-h-[90vh] flex flex-col">
              ...
              <div className="pt-6">
                ...
                <div className="space-y-2">
                  <label className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-colors ${paymentMethod === 'efectivo' ? 'border-black bg-gray-50' : 'border-gray-100 hover:border-gray-200'}`}>
                    ...
                    <div><p className="font-bold text-gray-900">💵 Pago en Efectivo</p><p className="text-xs text-gray-500">Pago en mostrador o al recibir.</p></div>
                  </label>
                  <label className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-colors ${paymentMethod === 'terminal' ? 'border-blue-500 bg-blue-50' : 'border-gray-100 hover:border-gray-200'}`}>
                    ...
                    <div><p className="font-bold text-blue-900">💳 Tarjeta / Terminal</p><p className="text-xs text-blue-700/80">Llevamos la terminal a tu mesa.</p></div>
                  </label>
                  <label className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-colors ${paymentMethod === 'link' ? 'border-sky-500 bg-sky-50' : 'border-gray-100 hover:border-gray-200'}`}>
                    ...
                    <div className="flex-1">
                      <p className="font-bold text-sky-900 flex justify-between"><span>🔗 Pago Seguro Online</span><span className="text-[10px] bg-sky-200 text-sky-800 px-2 py-0.5 rounded-full font-black">Mercado Pago</span></p>
            ...
            <div className="p-6 bg-white border-t border-gray-100">
              <button 
                ...
                className="w-full py-4 bg-black text-white font-black text-lg rounded-xl shadow-2xl hover:bg-gray-800 transition-colors disabled:opacity-50"
              >
                {isProcessingOrder ? "Generando Link Seguro..." : "Confirmar y Enviar Pedido"}
              </button>
            </div>
          </div>
        </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-gray-50 p-6 rounded-3xl border border-gray-100 shadow-inner flex flex-col items-center relative">
                    <div className="bg-white p-2 rounded-2xl shadow-sm mb-3">
                      <img src={`https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(customerProfile.phone)}`} alt="Tu QR VIP" className="w-40 h-40 object-contain" />
                    </div>
                    <p className="text-xs text-gray-400 font-bold uppercase tracking-widest mb-4">ID: {customerProfile.phone}</p>
                    <button onClick={() => window.open(`https://api.qrserver.com/v1/create-qr-code/?size=1000x1000&data=${encodeURIComponent(customerProfile.phone)}&margin=20`, "_blank")} className="text-xs font-black px-4 py-2 bg-blue-50 text-blue-600 rounded-full hover:bg-blue-100 transition-colors flex items-center gap-2">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg> Descargar QR
                    </button>
                  </div>
                  <div className="bg-black text-white p-5 rounded-2xl flex justify-between items-center shadow-lg">
                    <div><span className="font-bold text-sm text-gray-300 block">Saldo Actual</span><span className="font-medium text-xs">Muestra tu QR al cajero</span></div>
                          ...
                          return (
                            <div key={idx} className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm relative overflow-hidden">
                              <div className="flex justify-between items-end mb-2 relative z-10">
                                ...
                                <span className={`text-xs font-black px-2 py-1 rounded-full ${canRedeem ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{reward.pointsRequired} pts</span>
                              </div>
                              <div className="w-full bg-gray-100 h-2 rounded-full overflow-hidden relative z-10"><div className="h-full rounded-full transition-all duration-1000" style={{ width: `${progress}%`, backgroundColor: canRedeem ? '#10b981' : primaryColor }}></div></div>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm sm:p-4 animate-in fade-in duration-200">
          <div className="bg-gray-50 w-full sm:w-[400px] h-[85vh] sm:h-[600px] sm:rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-10 duration-300">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
              <div className="flex justify-start"><div className="bg-white text-gray-800 p-4 rounded-2xl rounded-tl-sm shadow-sm max-w-[85%] border border-gray-100"><p className="text-sm font-medium">¡Hola! 👋 Soy la Inteligencia Artificial de {businessName}. ¿En qué te puedo ayudar hoy?</p></div></div>
              {messages.map((msg, idx) => (
                <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`p-4 rounded-2xl shadow-sm max-w-[85%] text-sm font-medium ${msg.role === 'user' ? 'text-white rounded-tr-sm' : 'bg-white text-gray-800 border border-gray-100 rounded-tl-sm'}`} style={msg.role === 'user' ? { backgroundColor: primaryColor } : {}}>{msg.content}</div>
                </div>
              ))}
              {isTyping && (
                <div className="flex justify-start"><div className="bg-white p-4 rounded-2xl rounded-tl-sm shadow-sm border border-gray-100 flex gap-1"><span className="w-2 h-2 bg-gray-400 rounded-full animate-bounce"></span><span className="w-2 h-2 bg-gray-400 rounded-full animate-bounce" style={{ animationDelay: '0.2s' }}></span><span className="w-2 h-2 bg-gray-400 rounded-full animate-bounce" style={{ animationDelay: '0.4s' }}></span></div></div>
              )}
